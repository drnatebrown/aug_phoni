...
public:
    ...
    ms_rle_string(std::ifstream &heads, std::ifstream &lengths, ulint B = 2)
    {
        ...
        for (size_t i = 0; i < run_heads_s.size(); ++i)
        {
            ...
            std::fill_n(std::back_inserter(full_runs_bv), length - 1, false);
        ...
        full_runs = sparse_bitvector_t(full_runs_bv);
    ...
    // pair<ulint, ulint> run_start_pos(ulint i)
    // {
    //     ulint last_block = this->runs.rank(i);
    //     ulint current_run = last_block*this->B;
    //     //current position in the string: the first of a block
    //     ulint pos = 0;
    //     if(last_block>0)
    //         pos = this->runs.select(last_block-1)+1;
    //     assert(pos <= i);
    //     while(pos < i){
    ...
    //     }
    //     assert(pos>0);
    //     assert(current_run<this->R);
    //     return {current_run,pos-this->run_at(current_run)};
    // }
...
ms_rle_string<ri::sparse_sd_vector, ri::huff_string>::ms_rle_string(std::ifstream &heads, std::ifstream &lengths, ulint B)
{
    ...
    //runs in main bitvector
    vector<size_t> full_runs_bv_onset;
    size_t runs_bv_i = 0;
    // Compute runs_bv and runs_per_letter_bv
    for (size_t i = 0; i < run_heads_s.size(); ++i)
    {
        ...
        full_runs_bv_onset.push_back(this->n + length - 1);
    ...
    full_runs = ri::sparse_sd_vector(full_runs_bv_onset, this->n);
    //a fast direct array: char -> bitvector.
